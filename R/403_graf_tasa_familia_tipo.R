...
cols_fun <- colorRampPalette( c( 'white', 'gold', 'orange', 'olivedrab4',
                                 parametros$iess_green, 'turquoise2', 'deepskyblue2', 'steelblue2', 'blue2' ) )
cols_graf <- cols_fun( 100 )
...
aux <- cen_iess_hij_alis[ sexo == 'H' & sexo_dep == 'H' ]
# brewer.pal( 11, "Spectral" )
# wes_palette( "Zissou1", 50, type = "continuous" )
# viridis( 100 )
aux_obs <- cen_2010[ aseg == 'iess_sgo' & sexo == 'H' & sexo_dep == 'H' & tipo == 'H', list( x, y, q ) ]
aux_obs <- aux_obs[ q <= quantile( aux$q, probs = 0.9999 ) ]

plt_obs_hh <- ggplot() +
  geom_tile( data = aux_obs, aes( x = x, y = y, fill = q ), alpha = 0.8 ) +
...
plt_hh <- ggplot() +
  geom_tile( data = aux, aes( x = x, y = y, fill = q ), alpha = 0.8 ) +
...
aux <- data.table( res = cen_iess_hhh_2010$residuals )
...
plt_prob_hij_hh <- marrangeGrob( list( plt_obs_hh, plt_hh, plt_hist_hh, plt_qq_hh ), 
...
aux <- cen_iess_hij_alis[ sexo == 'H' & sexo_dep == 'M' ]
aux_obs <- cen_2010[ aseg == 'iess_sgo' & sexo == 'H' & sexo_dep == 'M' & tipo == 'H', list( x, y, q ) ]
aux_obs <- aux_obs[ q <= quantile( aux$q, probs = 0.9999 ) ]
...
aux <- data.table( res = cen_iess_hhm_2010$residuals )
...
plt_prob_hij_hm <- marrangeGrob( list( plt_obs_hm, plt_hm, plt_hist_hm, plt_qq_hm ), 
                                 layout_matrix = matrix( c( 1, 1, 1, 2, 2, 2,
                                                            1, 1, 1, 2, 2, 2,
                                                            1, 1, 1, 2, 2, 2,
                                                            NA, 3, 3, 4, 4, NA,
...
aux <- cen_iess_hij_alis[ sexo == 'M' & sexo_dep == 'H' ]
aux_obs <- cen_2010[ aseg == 'iess_sgo' & sexo == 'M' & sexo_dep == 'H' & tipo == 'H', list( x, y, q ) ]
aux_obs <- aux_obs[ q <= quantile( aux$q, probs = 0.9999 ) ]
...
aux <- data.table( res = cen_iess_mhh_2010$residuals )
...
plt_prob_hij_mh <- marrangeGrob( list( plt_obs_mh, plt_mh, plt_hist_mh, plt_qq_mh ), 
                                 layout_matrix = matrix( c( 1, 1, 1, 2, 2, 2,
                                                            1, 1, 1, 2, 2, 2,
                                                            1, 1, 1, 2, 2, 2,
                                                            NA, 3, 3, 4, 4, NA,
...
aux <- cen_iess_hij_alis[ sexo == 'M' & sexo_dep == 'M' ]
aux_obs <- cen_2010[ aseg == 'iess_sgo' & sexo == 'M' & sexo_dep == 'M' & tipo == 'H', list( x, y, q ) ]
aux_obs <- aux_obs[ q <= quantile( aux$q, probs = 0.9999 ) ]
...
aux <- data.table( res = cen_iess_mhm_2010$residuals )
...
plt_prob_hij_mm <- marrangeGrob( list( plt_obs_mm, plt_mm, plt_hist_mm, plt_qq_mm ), 
                                 layout_matrix = matrix( c( 1, 1, 1, 2, 2, 2,
                                                            1, 1, 1, 2, 2, 2,
                                                            1, 1, 1, 2, 2, 2,
                                                            NA, 3, 3, 4, 4, NA,
...
aux <- cen_iess_cony_alis

aux_obs <- as.data.table( cbind( cen_iess_cony$x, u = cen_iess_cony$y ) )
aux_obs[ , q := exp( u ) / ( 1 + exp( u ) ) ]
# aux_obs <- aux_obs[ q <= quantile( aux_obs$q, probs = 0.90 ) ]
...
aux <- data.table( res = cen_iess_cony$residuals )
...
plt_prob_cony <- marrangeGrob( list( plt_obs_cony, plt_cony, plt_hist_cony, plt_qq_cony ), 
                                 layout_matrix = matrix( c( 1, 1, 1, 2, 2, 2,
                                                            1, 1, 1, 2, 2, 2,
                                                            1, 1, 1, 2, 2, 2,
                                                            NA, 3, 3, 4, 4, NA,
