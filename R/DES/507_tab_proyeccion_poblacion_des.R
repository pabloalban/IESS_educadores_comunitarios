...
aux_f <- pob_proy[ sexo == 'F', list( t = t + parametros$anio_ini,
                                              l2_f = l2_cot_sgo+l2_ces,
                                              l2_cot_sgo_f=l2_cot_sgo,
                                              l2_cot_f = l2_cot, 
                                              l2_ces_f = l2_ces ) ]

aux_m <- pob_proy[ sexo == 'M', list( t = t + parametros$anio_ini,
                                                l2_m = l2_cot_sgo+l2_ces,
                                                l2_cot_sgo_m=l2_cot_sgo,
                                                l2_cot_m = l2_cot, 
                                                l2_ces_m = l2_ces ) ]
aux <- merge( aux_f, aux_m, by = c( 't' ) )
aux[, t := as.character( t ) ]
aux[ , l2 := l2_f + l2_m ]
aux[ , l2_cot_sgo := l2_cot_sgo_f + l2_cot_sgo_m ]
...
# Proyección de la población beneficiaría por pago--------------------------------------------------
...
aux <- merge( aux_f, aux_m, by = c( 't' ) )
aux[, t := as.character( t ) ]
aux_xtb <- xtable( aux, digits = c( 0, 0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2 ) )
print( aux_xtb,
       file = paste0( parametros$resultado_tablas, 'iess_tab_pob_proy_ben_pagos.tex' ),
       type = 'latex', 
       include.colnames = FALSE, include.rownames = FALSE, 
       format.args = list( decimal.mark = ',', big.mark = '.' ), 
       only.contents = TRUE, 
       hline.after = NULL, sanitize.text.function = identity )
# Proyección de la población beneficiaría por pago, con pagos indebidos-----------------------------
#pob_proy_pagos
...
xtb_aux <- xtable( aux, digits = c( 0, 0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2 ) )
print( xtb_aux,
       file = paste0( parametros$resultado_tablas, 'iess_tab_pob_proy_ben_pagos_ind.tex' ),
       type = 'latex', 
       include.colnames = FALSE, include.rownames = FALSE, 
       format.args = list( decimal.mark = ',', big.mark = '.' ), 
       only.contents = TRUE, 
