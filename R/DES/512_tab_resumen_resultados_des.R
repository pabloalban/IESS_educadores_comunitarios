...
load( paste0( parametros$RData_seg, 'IESS_DES_balances_',escenario[i],'.RData' ) )
aux <- balance_anual[ t == max(t), 
                        list( 
                          ...
                          l_p1=formatC(sum(balance$lp1),decimal.mark = ",",big.mark = ".", format='f',digits = 2),
                          V0=formatC(V0,decimal.mark = ",",big.mark = ".", format='f',digits = 2), 
                          A_vap=formatC(A_vap,decimal.mark = ",",big.mark = ".", format='f',digits = 2), 
                          activo = formatC(V0 + A_vap,decimal.mark = ",",big.mark = ".", format='f',digits = 2), 
                          ...
                          pri_med_niv_apo=formatC(prima$pri_med_niv_apo[length(prima$pri_med_niv_apo)]*100,decimal.mark = ",", format='f',digits = 5)
                          ) ]
  aux1 <- melt.data.table( aux, measure.vars = 1:ncol(aux) )
  aux2 <- data.table( 
                      i_a='Tasa actuarial (\\%)',
                      i_sbu='Tasa de crecimiento del SBU (\\%)',
                      l_p1='Beneficiarios del Pago 1 (2021 a 2060)',
                      V0 = 'Reserva inicial (USD)', 
                      A_vap = 'Aportes de afiliados (USD)', 
