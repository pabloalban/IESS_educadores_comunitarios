...
for ( i in 1:length( escenarios_lista ) ) {
  ...
  plt_bal_act <- ggplot() +
                geom_bar( data = balance_anual[V>=0], aes( x = t, y = V_cap ) , stat = 'identity', 
...
# grafico aportes y beneficios -------------------------------------------------------------------
  ...
  y_lim <- c( min(y_brk), max(y_brk) ) # redefiniendo limites por razones estéticas
  y_lbl <- formatC(y_brk/1e6, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',')
  balance_anual<-balance_anual[t>2018]
...
for ( i in 1:length( escenarios_lista ) ) {
  ...
  y_brk <- unique( pretty( seq(0, y_lim[2], length.out = 5) ) )
  y_lim <- c( 0, max(y_brk) ) # redefiniendo limites por razones estéticas
  y_lbl <- formatC(y_brk/1e6, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',')
