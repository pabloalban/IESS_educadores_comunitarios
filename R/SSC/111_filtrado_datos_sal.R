...
load( paste0( parametros$RData_seg, 'IESS_SAL_afiliados_2019.RData' ) )

setnames( afi_sal, 
          c( 'TIPO_ENFERMEDAD', 'TIP_SER', 'SEXO', 'CAPITULO', 'VSOL', 'ID_PACI' ), 
          c( 'enf', 'ser', 'sexo', 'cap', 'X', 'id' ) )

setnames( afi_sal_2019, 
          c( 'TIPO_ENFERMEDAD', 'TIP_SER', 'SEXO', 'CAPITULO', 'VSOL', 'ID_PACI' ), 
          c( 'enf', 'ser', 'sexo', 'cap', 'X', 'id' ) )
...
# Filtrando código ICD 2019
afi_sal_2019[ , icd := DIAG_PRINCIPAL ]
afi_sal_2019[ , icd := gsub( '[^[:alnum:]]', '', icd ) ]
afi_sal_2019[ , icd := str_trim( icd ) ]
afi_sal_2019 <- afi_sal_2019[ grepl( '^[A-Z]{1}', icd ) ]
afi_sal_2019 <- afi_sal_2019[ nchar( icd ) > 0 ]
afi_sal_2019[ , icd := substr( icd, 1, 3 ) ]
...
# Corrección fecha nacimiento 2019
afi_sal_2019[ , y := as.numeric( year( FEC_NAC ) ) ]
# afi_sal_2019[ y >= 0 & y <= 19, y := 2000 + y ]
# afi_sal_2019[ y >= 20 & y <= 99, y := 1900 + y ]
# afi_sal_2019[ , fec_nac := substr( FEC_NAC, 1, 7 ) ]
# afi_sal_2019[ , fec_nac := paste0( fec_nac, '-', y ) ]
afi_sal_2019[ , fec_nac := FEC_NAC ]
...
# Corrección fecha ingreso 2019
# afi_sal_2019[ , y := as.numeric( substr( FEC_ING, 8, 9 ) ) ]
# afi_sal_2019[ , y := 2000 + y ]
# afi_sal_2019[ , fec_ing := substr( FEC_ING, 1, 7 ) ]
# afi_sal_2019[ , fec_ing := paste0( fec_ing, '-', y ) ]
afi_sal_2019[ , fec_ing := FEC_ING ]
...
# Corrección fecha salida 2019
# afi_sal_2019[ , y := as.numeric( substr( FEC_SAL, 8, 9 ) ) ]
# afi_sal_2019[ , y := 2000 + y ]
# afi_sal_2019[ , fec_sal := substr( FEC_SAL, 1, 7 ) ]
# afi_sal_2019[ , fec_sal := paste0( fec_sal, '-', y ) ]
afi_sal_2019[ , fec_sal := FEC_SAL ]
gc()
...
# Tomando columnas necesarias 2019
afi_sal_2019 <- afi_sal_2019[ , list( y, id, sexo, enf, ser, cap, icd, fec_nac, fec_ing, fec_sal, X ) ]
gc()

# # Une historico con año 2019
# afi_sal <- rbind( afi_sal, afi_sal_2019 )
...
load( paste0( parametros$RData_seg, 'IESS_SAL_pensionistas_2019.RData' ) )
...
# Filtrando código ICD 2019
pen_sal_19[ , icd := DIAG_PRINCIPAL ]
pen_sal_19[ , icd := gsub( '[^[:alnum:]]', '', icd ) ]
pen_sal_19[ , icd := str_trim( icd ) ]
pen_sal_19 <- pen_sal_19[ grepl( '^[A-Z]{1}', icd ) ]
pen_sal_19 <- pen_sal_19[ nchar( icd ) > 0 ]
pen_sal_19[ , icd := substr( icd, 1, 3 ) ]
...
# Corrección fecha nacimiento 2019
pen_sal_19[ , y := as.numeric( year( FEC_NAC ) ) ]
# pen_sal_19[ y >= 0 & y <= 19, y := 2000 + y ]
# pen_sal_19[ y >= 20 & y <= 99, y := 1900 + y ]
# pen_sal_19[ , fec_nac := substr( FEC_NAC, 1, 7 ) ]
# pen_sal_19[ , fec_nac := paste0( fec_nac, '-', y ) ]
pen_sal_19[ , fec_nac := FEC_NAC ]
...
# Corrección fecha ingreso 2019
pen_sal_19[ , y := as.numeric( year( FEC_ING ) ) ]
# pen_sal_19[ , y := 2000 + y ]
# pen_sal_19[ , fec_ing := substr( FEC_ING, 1, 7 ) ]
# pen_sal_19[ , fec_ing := paste0( fec_ing, '-', y ) ]
pen_sal_19[ , fec_ing := FEC_ING ]
...
# Corrección fecha salida 2019
pen_sal_19[ , y := as.numeric( year( FEC_SAL ) ) ]
# pen_sal_19[ , y := 2000 + y ]
# pen_sal_19[ , fec_sal := substr( FEC_SAL, 1, 7 ) ]
# pen_sal_19[ , fec_sal := paste0( fec_sal, '-', y ) ]
pen_sal_19[ , fec_sal := FEC_SAL ]
gc()
...
pen_sal_19 <- pen_sal_19[ , list( y, id, sexo, enf, ser, cap, icd, fec_nac, fec_ing, fec_sal, X ) ]
gc()

# # Une historico con el anio 2019
# pen_sal <- rbind( pen_sal, pen_sal_19 )
...
load( paste0( parametros$RData_seg, 'IESS_SAL_hijos_2019.RData' ) )

hijos_sal_19 <- hij_sal_19
rm( hij_sal_19 )

hijos_sal_19 <- hijos_sal_19[ , !'ID_AFIL' ]

setnames( hijos_sal, 
          c( 'TIP_ENF', 'TIP_SER', 'SEXO', 'CAPITULO', 'FAC', 'ID_HIJO' ), 
          c( 'enf', 'ser', 'sexo', 'cap', 'X', 'id' ) )

setnames( hijos_sal_19, 
          c( 'TIPO_ENFERMEDAD', 'TIP_SER', 'SEXO', 'CAPITULO', 'VSOL', 'ID_PACI', 'DIAG_PRINCIPAL' ), 
          c( 'enf'            , 'ser'    , 'sexo', 'cap', 'X', 'id', 'DIAG' ) )

# View(head(hijos_sal_19,1000))
# View(head(hijos_sal,1000))
# colnames(hijos_sal)
# colnames(hijos_sal_19)
...
# Filtrando código ICD 2019
hijos_sal_19[ , icd := DIAG ]
hijos_sal_19[ , icd := gsub( '[^[:alnum:]]', '', icd ) ]
hijos_sal_19[ , icd := str_trim( icd ) ]
hijos_sal_19 <- hijos_sal_19[ grepl( '^[A-Z]{1}', icd ) ]
hijos_sal_19 <- hijos_sal_19[ nchar( icd ) > 0 ]
hijos_sal_19[ , icd := substr( icd, 1, 3 ) ]
...
# Corrección fecha nacimiento 2019
hijos_sal_19[ , y := as.numeric( year( FEC_NAC ) ) ]
# hijos_sal_19[ y >= 0 & y <= 19, y := 2000 + y ]
# hijos_sal_19[ y >= 20 & y <= 99, y := 1900 + y ]
# hijos_sal_19[ , fec_nac := substr( FEC_NAC, 1, 7 ) ]
# hijos_sal_19[ , fec_nac := paste0( fec_nac, '-', y ) ]
hijos_sal_19[ , fec_nac := FEC_NAC  ]
...
# Corrección fecha ingreso 2019
hijos_sal_19[ , y := as.numeric( year( FEC_ING ) ) ]
# hijos_sal_19[ , y := 2000 + y ]
# hijos_sal_19[ , fec_ing := substr( FEC_ING, 1, 7 ) ]
# hijos_sal_19[ , fec_ing := paste0( fec_ing, '-', y ) ]
hijos_sal_19[ , fec_ing := FEC_ING ]
...
# Corrección fecha salida 2019
hijos_sal_19[ , y := as.numeric( year( FEC_SAL ) ) ]
# hijos_sal_19[ , y := 2000 + y ]
# hijos_sal_19[ , fec_sal := substr( FEC_SAL, 1, 7 ) ]
# hijos_sal_19[ , fec_sal := paste0( fec_sal, '-', y ) ]
hijos_sal_19[ , fec_sal := FEC_SAL ]
gc()

# Tomando columnas necesarias
hijos_sal <- hijos_sal[ , list( y, id, sexo, enf, ser, cap, icd, fec_nac, fec_ing, fec_sal, X ) ]
gc()
hijos_sal_19 <- hijos_sal_19[ , list( y, id, sexo, enf, ser, cap, icd, fec_nac, fec_ing, fec_sal, X ) ]
gc()


# # Une historico con anio 2019
# hijos_sal <- rbind( hijos_sal, hijos_sal_19 )
