...
escenarios_lista <- paste0( 'escenario_', 1:5 ) # 5 escenarios a oct 11, 2019
...
for ( i in 1:length( escenarios_lista ) ) { # i <- 1
  ...
  balance_anual[ , t := t + parametros$anio]
  ...
  ggsave( plot = plt_bal_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_balance_capitalizado_', escenario,'_ssc', parametros$graf_ext ),
  ...
  ggsave( plot = plt_apo_ben_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_apo_ben_bal_capitalizado_', escenario,'_ssc', parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
}
...
for ( i in 1:length( escenarios_lista ) ) { # i<-1
  ...
  ggsave( plot = plt_bal_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_balance_vap_', escenario,'_ssc', parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
  
  #Modificaciones propuestas por Ramiro Vega
  min(balance_anual$V )
  max(balance_anual$V )
  
  m <- ifelse(  max( balance_anual$V ) > 0 &  min ( balance_anual$V ) > 0, 0 , min( balance_anual$V  ) )
  
  y_lim <- c( m, max( balance_anual$V ) )
  y_brk <- unique( pretty( seq(y_lim[1], y_lim[2], length.out = 7) ) )
  y_lim <- c( min(y_brk), max(y_brk) ) # redefiniendo limites por razones estéticas
  y_lbl <- formatC(y_brk/1e6, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',')
  ...
  plt_bal_act <- ggplot() +
    ...
    geom_bar( data = balance_anual[V<0], aes( x = t, y = V ) , stat = 'identity', 
              fill = 'red',  width = 0.5 ) +
    geom_hline( aes( yintercept = 0 ), size = 0.5*graf_line_size, 
                color = parametros$iess_green, linetype = 2 ) +
    xlab( 'Año') +
    ylab( 'Balance actuarial dinámico (millones)' ) +
  ...
  ggsave( plot = plt_bal_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_balance_vap_rv_', escenario,'_ssc', parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
  ...
  ggsave( plot = plt_apo_ben_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_apo_ben_bal_dinamico_', escenario,'_ssc', parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
}
