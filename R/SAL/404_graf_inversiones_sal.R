...
aux<-aux[aux$Periodo > ymd('2011-12-31', tz = parametros$time_zone )]
aux<-as.data.frame(aux)
aux['intrumento']<-'Saldo valor nominal'
aux<-select(aux,Periodo,intrumento,inversiones,rendimiento_neto)
...
df_line = aux %>% select(ano, Rendimiento)
...
df_bar <- aux %>% select(-rendimiendo, -anio)
df_line = aux %>% select(Periodo, rendimiendo)
