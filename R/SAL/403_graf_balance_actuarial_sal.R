...
escenarios_lista <- paste0( 'escenario_', 1:4 ) # 2 escenarios para el actualizado
...
for ( i in 1:length( escenarios_lista ) ) { # i <- 1
  ...
  ggsave( plot = plt_bal_cap, 
          filename = paste0( parametros$resultado_graficos, 'iess_balance_capitalizado_', escenario, parametros$graf_ext ),
  ...
  ggsave( plot = plt_apo_ben_cap, 
          filename = paste0( parametros$resultado_graficos, 'iess_apo_ben_bal_capitalizado_', escenario, parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
  ...
  ggsave( plot = plt_bal_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_balance_actuarial_', escenario, parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
...
#   load( paste0( parametros$RData_seg, 'IESS_SAL_balances_', escenario, '.RData' ) )
#   
#   num_anios <- length( unique( agregado_financiero$Year ) )
#   cols_fun <- colorRampPalette( c( 'gold', parametros$iess_green, parametros$iess_blue ) )
#   cols_graf <- cols_fun( num_anios )
#   
#   agregado_financiero[ , t := Year ]
#   
#   # grafico balance capitalizado -------------------------------------------------------------------
#   x_lim <- c( parametros$anio_ini, parametros$anio_ini + parametros$horizonte )
#   x_brk <- seq( x_lim[1], x_lim[2], 5 )
#   x_lbl <- formatC( x_brk, digits = 0, format = 'f' )
#   
#   y_lim <- c( min( agregado_financiero$V ), max( agregado_financiero$V ) )
#   y_brk <- unique( pretty( seq(y_lim[1], y_lim[2], length.out = 5) ) )
#   y_lim <- c( min(y_brk), max(y_brk) ) # redefiniendo limites por razones estéticas
#   y_lbl <- formatC(y_brk/1e6, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',')
#   
#   plt_bal_cap <- ggplot() +
#     geom_line( data = agregado_financiero, aes( x = Year, y = V ), 
#                size = graf_line_size, color = parametros$iess_blue ) +
#     geom_hline( aes( yintercept = 0 ), size = 0.5*graf_line_size, color = parametros$iess_green, linetype = 2 ) +
#     xlab( 'Año') +
#     ylab( 'Balance Capitalizado (millones)' ) +
...
#   
#   # grafico aportes y beneficios -------------------------------------------------------------------
#   y_lim <- c( min( agregado_financiero[ , list( aporte_tot_vap, B_tot_vap ) ] ),
#               max( agregado_financiero[ , list( aporte_tot_vap, B_tot_vap ) ] )
#               )
#   y_brk <- unique( pretty( seq(y_lim[1], y_lim[2], length.out = 5) ) )
#   y_lim <- c( min(y_brk), max(y_brk) ) # redefiniendo limites por razones estéticas
#   y_lbl <- formatC(y_brk/1e6, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',')
...
#   # aux1 <- balance_anual[ t > parametros$anio_ini, list(item = 'Aportes', t = t, valor = A) ]
#   # aux2 <- balance_anual[ t > parametros$anio_ini, list(item = 'Beneficios', t = t, valor = B) ] 
#   aux1 <- agregado_financiero[ , list( item = 'Aportes', t = Year, valor = aporte_tot_vap ) ]
#   aux2 <- agregado_financiero[ , list( item = 'Beneficios', t = Year, valor = B_tot_vap ) ] 
#   
#   aux <- rbind( aux1, aux2 )
#   plt_apo_ben_cap <- ggplot( aux, aes( x = t, y = valor ) ) +
#     geom_line( aes(colour = item), size = graf_line_size ) +
