...
x_lim <- c( 2005, 2020 )
x_brk <- seq( x_lim[1], x_lim[2],by= 3 )
...
x_lim <- c( 2005, 2020 )
x_brk <- seq( x_lim[1], x_lim[2],by = 1 )
x_lbl <- formatC( x_brk, digits = 0, format = 'f' )
...
iess_masa_salarial_ini <- ggplot( data = aux ) + 
  ...
  theme( #legend.position = "bottom",
         legend.direction = "vertical",
         axis.text.x = element_text( angle = 0, hjust = 1), legend.text = element_text(size = 6, colour = "black"))+
...
ggsave( plot = iess_masa_salarial_ini,
        ...
        width = graf_width + 7, height = graf_height, units = graf_units, dpi = graf_dpi )
...
# Graficos hijos menores de 18 años --------------------------------------------
unidad<-1e6
...
y_lim <- c( 500000, 1500000)
y_brk <- seq( y_lim[1], y_lim[2], by = 200000 )
y_lbl <- formatC( y_brk, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',' )


iess_pob_hijos <- ggplot( data = aux ) + 
  geom_line( aes( x = anio, 
                  y = hijos_h, colour = "Masculino" ), size = graf_line_size  ) + 
  geom_line( aes( x = anio, 
                  y = hijos_m, colour = "Femenino" ), size = graf_line_size  ) + 
  # geom_line( aes( x = anio, 
  #                 y = Afiliados, colour = "Afiliados" ), size = graf_line_size  ) + 
  scale_colour_manual( "",
  ...
  theme_bw() +
  # plt_theme +
  labs(  x = 'Año', y = 'Hijos mejores de 18 años' ) +
  theme( #legend.position =  "bottom",
    legend.direction = "vertical",
    axis.text.x = element_text( angle = 0, hjust = 1,face="bold"), 
...
#iess_pob_hijos
ggsave( plot = iess_pob_hijos,
        filename = paste0( parametros$resultado_graficos, 'iess_pob_hijos', parametros$graf_ext ),
        width = graf_width + 5, height = graf_height, units = graf_units, dpi = graf_dpi )

# Gráficos de la pirámide de hijos menores de 18 años _-------------------------
message( '\tGraficando piramide hijos menores de 18 años' )

aux <- copy( pob_hijos_18_edad )
max_edad <- 17
min_edad <- 0

aux1 <- aux[ , list( sexo = 'H', edad, n = hijos_h ) ]
aux2 <- aux[ , list( sexo = 'M', edad, n = hijos_m ) ]
aux <- rbind( aux1, aux2 )
aux[is.na(n),n:=0]  #reemplazo datos NA por cero
...
iess_pir_hijos_18 <- ggplot(aux, aes(x = edad, y = n, fill= sexo)) +
  xlab( 'Edad' ) +
  ylab( '' ) +
  geom_bar( data = aux[ sexo == 'M' ], stat = 'identity',colour="white", size=0.1) +
  geom_bar( data = aux[ sexo == 'H' ], stat = 'identity',colour="white", size=0.1) +
...
ggsave( plot = iess_pir_hijos_18,
        filename = paste0( parametros$resultado_graficos, 'iess_pir_hijos_18', parametros$graf_ext ),
        width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
