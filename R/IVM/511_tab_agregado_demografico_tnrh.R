...
for(i in 1:length( escenarios_lista )) #i=1
{
  ...
  load(paste0( parametros$RData_seg, 'IESS_TNRH_agregado_demografico_', escenario, '.RData' ))
  ...
  print( xtb_aux,
         file = paste0( parametros$resultado_tablas, 'iess_tnrh_agregado_demografico_f_', escenario, '.tex' ),
         type = 'latex', 
  ...
  print( xtb_aux,
         file = paste0( parametros$resultado_tablas, 'iess_tnrh_agregado_demografico_m_', escenario, '.tex' ),
         type = 'latex', 
         include.colnames = FALSE, include.rownames = FALSE, 
         format.args = list( decimal.mark = ',', big.mark = '.' ), 
         only.contents = TRUE, 
