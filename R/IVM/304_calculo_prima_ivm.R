...
for (i in 1:length(escenarios_lista)) #i=2
{
  ...
  load( paste0( parametros$RData_seg, 'IESS_IVM_balances_', escenarios_lista[i], '.RData' ) )
...
#  prima[ , pri_med_niv_delta_pen := delta * B_dec_vap / M_vap ]
  ...
  save( prima, 
        file = paste0( parametros$RData_seg, 'IESS_IVM_primas_', escenarios_lista[i], '.RData' ) )  
}
