...
escenarios_lista <- paste0( 'escenario_', 1:2 ) # 2 escenarios 
...
for ( i in 1:length( escenarios_lista ) ) {
  ...
  plt_bal_act <- ggplot() +
    ...
    geom_hline( aes( yintercept = 0 ), size = 0.5 * graf_line_size, color = parametros$iess_green,
                linetype = 2 ) +
    xlab( 'Año') +
    ylab( 'Balance Capitalizado (millones USD)' ) +
  ...
  # Gráfico del balance capitalizado con barras--------------------------------------------------------
  x_lim <- c( parametros$anio_ini, parametros$anio_ini + parametros$horizonte )
  x_brk <- seq( x_lim[1], x_lim[2], 5 )
  x_lbl <- formatC( x_brk, digits = 0, format = 'f' )
  ...
  plt_bal_act <-  ggplot() +
    ...
    geom_hline( aes( yintercept = 0 ), 
                ...
                linetype = 2 ) +
    xlab( 'Año') +
    ylab( 'Balance actuarial dinámico (millones USD)' ) +
  ...
  # Gráfico aportes y beneficios capitalizado-------------------------------------------------------
  y_lim <- c( min( balance_anual[ , list(A, B) ] ), max( balance_anual[ , list(A, B) ] ) )
  y_brk <- unique( pretty( seq(y_lim[1], y_lim[2], length.out = 5) ) )
  y_lim <- c( min(y_brk)+1, max(y_brk) ) # redefiniendo limites por razones estéticas
  y_lbl <- formatC( y_brk/1e6, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',' )
  ...
  plt_apo_ben_act <- ggplot( aux, aes( x = t, y = valor ) ) +
    ...
    xlab( 'Año' ) +
    ylab( 'Aportes y Beneficios Balance Capitalizado (millones USD)' ) +
    scale_y_continuous( limits = y_lim, breaks = y_brk, labels = y_lbl ) +
  ...
  # Gráfico de los saldos obligaciones de cuentas individuales--------------------------------------
  x_lim <- c( parametros$anio_ini, parametros$anio_ini + parametros$horizonte )
  x_brk <- seq( x_lim[1], x_lim[2], 5 )
  x_lbl <- formatC( x_brk, digits = 0, format = 'f' )
  
  y_lim <- c( 0, max( balance_anual$C ) )
  y_brk <- unique( pretty( seq(y_lim[1], y_lim[2], length.out = 5) ) )
  y_lim <- c( min(y_brk), max(y_brk) ) # redefiniendo limites por razones estéticas
  y_lbl <- formatC( y_brk/1e6, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',' )
  
  plt_bal_act <-  ggplot() +
    geom_bar( data = balance_anual[ C >= 0 ], aes( x = t, y = C ), 
              stat = 'identity', fill = parametros$iess_blue, width = 0.5) +
    geom_bar( data = balance_anual[ C < 0 ], aes( x = t, y = C ), 
              stat = 'identity', fill = 'red', width = 0.5) +
    geom_hline( aes( yintercept = 0 ), 
                ...
                linetype = 2 ) +
    xlab( 'Año') +
    ylab( 'Obligaciones por cuentas individuales (millones USD)' ) +
    scale_x_continuous( breaks = x_brk, labels = x_lbl ) +
    scale_y_continuous( breaks = y_brk, labels = y_lbl  ) +
    theme_bw() +
    plt_theme
  ...
  ggsave( plot = plt_bal_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_saldos_cuentas_cap_',
                             escenario, parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
}
...
for ( i in 1:length( escenarios_lista ) ) {
  ...
  plt_apo_ben_act <- ggplot( aux, aes( x = t, y = valor ) ) +
    geom_line( aes(colour = item), size = graf_line_size ) +
    xlab( 'Año' ) +
    ylab( 'Aportes y Beneficios Balance Dinámico (millones USD)' ) +
    scale_y_continuous( limits = y_lim, breaks = y_brk, labels = y_lbl ) +
    scale_x_continuous( breaks = x_brk, labels = x_lbl, limits = x_lim ) +
    theme_bw() +
    scale_colour_manual( values = c('Aportes' = parametros$iess_blue,
  ...
  ggsave( plot = plt_apo_ben_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_apo_ben_bal_dinamico_', escenario,
                             parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
  # Gráfico de los saldos obligaciones de cuentas individuales en valor actual----------------------
  x_lim <- c( parametros$anio_ini, parametros$anio_ini + parametros$horizonte )
  x_brk <- seq( x_lim[1], x_lim[2], 5 )
  x_lbl <- formatC( x_brk, digits = 0, format = 'f' )
  
  y_lim <- c( 0, max( balance_anual$C_vap ) )
  y_brk <- unique( pretty( seq(y_lim[1], y_lim[2], length.out = 5) ) )
  y_lim <- c( min(y_brk), max(y_brk) ) # redefiniendo limites por razones estéticas
  y_lbl <- formatC( y_brk/1e6, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',' )
  
  plt_bal_act <-  ggplot() +
    geom_bar( data = balance_anual[ C_vap >= 0 ], aes( x = t, y = C_vap ), 
              stat = 'identity', fill = parametros$iess_blue, width = 0.5) +
    geom_bar( data = balance_anual[ C_vap < 0 ], aes( x = t, y = C_vap ), 
              stat = 'identity', fill = 'red', width = 0.5) +
    geom_hline( aes( yintercept = 0 ), 
                ...
                linetype = 2 ) +
    xlab( 'Año') +
    ylab( 'Obligaciones por cuentas individuales, en valor actual (millones)' ) +
    scale_x_continuous( breaks = x_brk, labels = x_lbl ) +
    scale_y_continuous( breaks = y_brk, labels = y_lbl  ) +
    theme_bw() +
    plt_theme
  ...
  ggsave( plot = plt_bal_act, 
          filename = paste0( parametros$resultado_graficos, 'iess_saldos_cuentas_vap_',
                             escenario, parametros$graf_ext ),
          width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
}
